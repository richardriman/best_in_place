/*
  ...
  If you want textarea autosizing, there is dependency to autosize.js (http://www.jacklmoore.com/autosize/).
*/
...
BestInPlaceEditor.prototype = {
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function() {
      ...
      self.taxtareaAutosize  = self.textareaAutosize  || $parent.attr("data-autosize");
    });
    ...
    self.taxtareaAutosize  = self.element.attr("data-autosize")            || self.textareaAutosize  || false;
...
BestInPlaceEditor.forms = {
  "input" : {
    activateForm : function() {
      var output = jQuery('<form/>')
                   .addClass('form_in_place')
                   .attr('action', 'javascript:void(0);')
                   .attr('style', 'display:inline');
      var input_elt = jQuery('<input/>')
                      .attr('type', 'text')
  ...
  "select" : {
    activateForm : function() {
      var output     = jQuery('<form/>')
          ...
          select_elt = jQuery('<select/>')
                       .addClass(this.inner_class !== null ? this.inner_class : '' ),
          currentCollectionValue = this.collectionValue;

      jQuery.each(this.values, function (index, value) {
        var option_elt = jQuery('<option/>')
                         .val(value[0])
                         .html(value[1]);
  ...
  "textarea" : {
    activateForm : function() {
      ...
      if (this.textareaAutosize) {
        jQuery(this.element.find("textarea")[0]).autosize({append: "\n"});
      }
